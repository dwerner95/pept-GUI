...
class PeptMl(QDialog):
    ...
    def ok_button_clicked(self):
        ...
        time_ = time.time()
        self.parent.pept_pipeline[f"HDBSCAN_{time_}"] = {
            "function": tracking.HDBSCAN,
            "name": "tracking.HDBSCAN",
            "kwargs": {
                "true_fraction": true_fraction,
                "max_tracers": self.parent.maxtracer,
            },
            "update": True,
            "skip": False,
        }
        ...
        self.parent.pept_pipeline[f"split_{time_}"] = {
            "function": lambda error: tracking.SplitLabels()
            + tracking.Centroids(error=error),
            "name": "tracking.SplitLabels() + tracking.Centroids",
            "kwargs": {"error": calculate_error},
            "update": True,
            "skip": False,
        }
    ...
    def help_button_clicked(self):
        ...
        self.parent.PlotlyPlotRegion.setUrl(
            QUrl(
                "https://pept.readthedocs.io/en/latest/manual/generated/pept.tracking.HDBSCAN.html?highlight=HDBscan"
            )
        )
        self.close()
